...
import 'package:altx2/models/user.dart';
...
class UserNotifier extends StateNotifier<LocalUser> {
  ...
  Future<void> login(String email) async {
    ...
    if (response.docs.length != 1) {
      print("More than one firestore user associate with this email: $email");
      return;
    }
    state = LocalUser(
  ...
  Future<void> signUp(String email) async {
    DocumentReference response = await _firestore.collection("users").add(
        FirebaseUser(
                ...
                name: "No Name",
                profilePic:
                    "https://wallpapers-clan.com/wp-content/uploads/2023/01/one-punch-man-saitama-pfp-8.jpg")
            .toMap());
